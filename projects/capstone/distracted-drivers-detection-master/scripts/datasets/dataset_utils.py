...
def _get_test_filenames(dataset_dir):
    image_filenames = []
    for file in os.listdir(dataset_dir):
        image_filename = os.path.join(dataset_dir, file)
...
def _dataset_exists(dataset_dir, _NUM_SHARDS, output_filename):
  ...
  for i in range(3):
    ...
    num_shards = _NUM_SHARDS[i]
    for shard_id in range(num_shards):
      tfrecord_filename = _get_dataset_filename(
          dataset_dir, split_name, shard_id, output_filename, num_shards)
